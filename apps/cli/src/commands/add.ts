...
export const addCommand = workspaceProcedure
  ...
  .input(inputSchema)
  .query(
    async ({ input: [inputWorkspacePackageType, inputPackageName], ctx }) => {
      const inputPackageTypePackages =
        inputWorkspacePackageType == 'app'
          ? appTypes
          : inputWorkspacePackageType == 'package'
            ? packageTypes
            : inputWorkspacePackageType == 'tool'
              ? toolTypes
              : undefined;
      const inputPackageType =
        inputPackageName &&
        (inputPackageTypePackages as readonly string[]).includes(
          inputPackageName,
        )
          ? (inputPackageName as AppType | PackageType | ToolType)
          : undefined;

      const promptInput = await prompts.group({
        workspacePackageType: () =>
          inputWorkspacePackageType
            ? Promise.resolve(inputWorkspacePackageType)
            : prompts.select({
        ...
        type: ({ results: { workspacePackageType } }) => {
          ...
          if (inputPackageType) {
            return Promise.resolve(inputPackageType);
          }
